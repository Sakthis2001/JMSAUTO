...
//--General--//
    ...
    //AddWorkflow
    private String workflowmenu="//p[text()='Workflow']";
    private  String addworkflowbutton="//button[text()='Add New Workflow']";
    private  String loginstage="//div[text()='Stages']//following::div[text()='Login ']";
    private  String latexexecutivestage="//div[text()='Stages']//following::div[text()='LaTeX Executive ']";
    private  String graphicsstage="//div[text()='Stages']//following::div[text()='Graphics ']";
    private  String preeditingstage="//div[text()='Stages']//following::div[text()='Pre Editing ']";
    private String target="//div[text()='Stages']//following::div[@class='react-flow']";
    ...
    public void DoAddArticle(String journalacro, String articleid, String artname, String doinum, String workflow) {
        ...
        page.locator(articleidinput).fill("9025949585");
...
//page.locator(checkall).click();
    ...
    private static void performDragAndDrop(Page page, Locator source, Locator target) {
        // Get the bounding box of the source element
        BoundingBox sourceBoundingBox = source.boundingBox();
        if (sourceBoundingBox == null) {
            throw new RuntimeException("Source element not found");
        }

        // Get the bounding box of the target element
        BoundingBox targetBoundingBox = target.boundingBox();
        if (targetBoundingBox == null) {
            throw new RuntimeException("Target element not found");
        }

        // Perform the drag-and-drop operation
        page.mouse().move(sourceBoundingBox.x + sourceBoundingBox.width / 2, sourceBoundingBox.y + sourceBoundingBox.height / 2);
        page.mouse().down();
        page.mouse().move(targetBoundingBox.x + targetBoundingBox.width / 2, targetBoundingBox.y + targetBoundingBox.height / 2);
        page.mouse().up();
    }

   /* public void CreateWorkflow() throws InterruptedException {

        page.locator(workflowmenu).click();
        page.locator(addworkflowbutton).click();

        Locator loginsource=page.locator(loginstage);
        Locator latexsource=page.locator(latexexecutivestage);
        Locator graphicssource=page.locator(graphicsstage);
        Locator preeditsource=page.locator(preeditingstage);

        Locator logintarget=page.locator("//div[@style='z-index: 1010; transform: translate(332.567px, 132.733px); pointer-events: all; visibility: visible;']");
        Locator latextarget=page.locator("//div[@style='z-index: 1010; transform: translate(191.867px, 213.033px); pointer-events: all; visibility: visible;']");
        Locator graphicstarget=page.locator("//div[@style='z-index: 1010; transform: translate(478.867px, 203.033px); pointer-events: all; visibility: visible;']");
        Locator preedittarget=page.locator("//div[@style='z-index: 1010; transform: translate(377.867px, 277.533px); pointer-events: all; visibility: visible;']");



        performDragAndDrop(page, loginsource, logintarget);
        performDragAndDrop(page, latexsource, latextarget);
        performDragAndDrop(page, graphicssource, graphicstarget);
        performDragAndDrop(page, preeditsource, preedittarget);
