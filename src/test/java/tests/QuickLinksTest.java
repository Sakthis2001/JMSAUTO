...
public class QuickLinksTest extends BaseTest {
    ...
    @Test(priority =1, description = "JMS-68 verify Quicklinks is available from that page and working fine from every page-version 3")
    ...
    @Test(priority =2, description = "JMS-66:Verify the quicklinks displays only Add article for Login user, Add Pub,Journal is not available for login user- version 3")
    public void EnsureLoginUserHasOnlyAddArticleIcon()
    {
        ...
        softAssert.assertFalse(visiblity.get(0),"Add article icon is not displayed ");
        softAssert.assertFalse(visiblity.get(1),"pubIcon is Dispplayed In Login User");
        softAssert.assertTrue(visiblity.get(2),"journalIcon Is Displayed In Login User");
    ...
    @Test(priority =3,dataProvider ="usercredentials", description = "JMS-67,Verify apart from PM, login users, quicklinks is not available for other users-version 3")
